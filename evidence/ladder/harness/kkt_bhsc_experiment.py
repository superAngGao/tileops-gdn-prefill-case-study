"""Experiment-only FlashQLA-style KKT prepare with contiguous BHSC output.

This keeps the FlashQLA KKT/block triangular inverse schedule, but writes the
64x64 A tile to a physically contiguous ``[B, H, T, C]`` tensor.  The production
TileOps replay ABI still expects ``[B, T, H, C]``, so callers that feed replay
must materialize the final BTHC layout explicitly and count that cost.
"""
...
try:
    from flash_qla.tilelang_compat import install_gemm_v1_compat
...
def _tilelang_kkt_solve_bhsc(
    ...
    a_shape = (data_batch_size, H, num_tokens, chunk_size)
    ...
    def _kernel():
        @T.prim_func
        def tilelang_kkt_solve_bhsc_kernel(
            k: T.Tensor(k_shape, dtype=qkva_dtype),
            b: T.Tensor(b_shape, dtype=b_dtype),
            a: T.Tensor(a_shape, dtype=qkva_dtype),
            num_chunks_arg: T.int32,
        ):
            with T.Kernel(num_chunks_arg * H, threads=256) as (bch,):
                bc, bh = bch // H, bch % H
                bhg = bh // (H // Hg)
                ...
                if tx < 128:
                    ...
                    for j_s, k_s, k_t in T.Parallel(2, 32, 32):
                        if k_s >= 16 and k_t < 16:
                            a32i_fragment[j_s, k_s, k_t] = a16o_shared[
                                j_s, k_s - 16, k_t
                            ]
                    for j_s, k_s, k_t in T.Parallel(2, 32, 32):
                        if k_s // 16 == k_t // 16:
                    ...
                    if tx < 160:
                        ...
                        if right <= seq_end_idx:
                            T.copy(a64_shared, a[bb, bh, left:right, 0:block_S])
                    else:
                        T.barrier_wait(a_is_ready, 0)
                        if right > seq_end_idx:
                            for j_s, j_t in T.Parallel(block_S, block_S):
                                if left + j_s < seq_end_idx:
                                    a[bb, bh, left + j_s, j_t] = a64_shared[j_s, j_t]

        return tilelang_kkt_solve_bhsc_kernel

    return _kernel()


def kkt_solve_bhsc(k: torch.Tensor, b: torch.Tensor, chunk_size: int = 64) -> torch.Tensor:
    batch_size, num_tokens, Hg, K = k.shape
    _, _, H = b.shape
    assert K == 128
    assert chunk_size == 64
    num_chunks = batch_size * tilelang.cdiv(num_tokens, chunk_size)
    a = torch.empty((batch_size, H, num_tokens, chunk_size), dtype=k.dtype, device=k.device)
    kernel = _tilelang_kkt_solve_bhsc(
        H,
        Hg,
        K,
        chunk_size,
...
def kkt_prepare_bthc(k: torch.Tensor, b: torch.Tensor, chunk_size: int = 64) -> torch.Tensor:
    return kkt_solve_bhsc(k, b, chunk_size).permute(0, 2, 1, 3).contiguous()
